...
def create_data_structure(string_input):
    ...
    if not string_input == "":
        ...
        for i in range(0,len(info)-2,2):
            line = info[i]
            friends_list = line.split(' is connected to ')
            name = friends_list[0]
            friends = friends_list[1].replace(" ","").split(',')
            line = info[i+1]
            games_list = line.split(' likes to play ')
...
def add_connection(network, user_A, user_B):
    if user_A in network and user_B in network:
        friends_A = network[user_A][0]
        if not user_B in friends_A:
            friends_A.append(user_B)
            return network
    else: 
        return False
    return network
...
def get_secondary_connections(network, user):
    ...
    for connection in connections:
        cons = get_connections(network, connection)
        for c in cons:
            if c not in sec_connections:
                sec_connections.append(c)
    ...
    if user_B in friends:
        ...
        for friend in friends:
            if friend not in visited:
                path = find_path_to_friend(network, friend, user_B, visited)
                if path:
